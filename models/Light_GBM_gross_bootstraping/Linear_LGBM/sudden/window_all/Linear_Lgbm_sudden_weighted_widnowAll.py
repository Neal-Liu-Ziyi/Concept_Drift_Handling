...
for row in range(len(sudden_df)):
    ...
    training_set_all = sudden_series_df.iloc[:round(0.8 * len(sudden_series_df))]
    test_set = sudden_series_df.iloc[round(0.8 * len(sudden_series_df)):]

    # trainign with fixed window
    training_set = training_set_all.iloc[:]
    ...
    prediction_mean_df['drift_point' + str(drift_point)] = predictions
...
sudden_df.to_csv('Linear_sudden_cd_weighted_cv8_window_All.csv',index=False)
prediction_mean_df.to_csv('Linear_sudden_prediction_weighted_cv8_window_All.csv',index=False)